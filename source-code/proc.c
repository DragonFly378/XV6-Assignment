...
{
  int tmp_ticks = ticks - (p->start_ticks);
  if (tmp_ticks < 1000)
    cprintf("%d\t%s\t       0.%d\t%s\t%d\t ", p->pid, p->name, tmp_ticks, states[p->state], p->sz);
  else{
    int tmp = tmp_ticks % 1000;
    tmp_ticks = tmp_ticks / 1000;
    cprintf("%d\t%s\t       %d.%d\t%s\t%d\t ", p->pid, p->name, tmp_ticks, tmp, states[p->state], p->sz);
  }
  return;
}
#endif
